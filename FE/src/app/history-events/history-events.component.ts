...
export class HistoryEventsComponent implements OnInit {
  ...
  constructor(
    private historicalEventService: HistoryEventsService,
  ...
  public deleteItemConfirmed(): void {
    this.showDeleteConfirmationModal = false;
    this.historicalEventService
      .deleteHistoricalEvent(this.eventToDelete)
      .subscribe(
        () => {
  ...
  private onUpdateHistoryEvent(updateEvent: HistoryEvents): void {
    this.historicalEventService.updateHistoricalEvent(updateEvent).subscribe(
      () => {
        const index = this.historicalEvents.findIndex(
