...
@Input() newFeatureForm:any;
 constructor(private geographyFeaturesService:GeographyService, 
 ...
 private getGeographicalFeatures() :void {
        this.geographyFeaturesService.getGeographicalFeatures().subscribe(
          (features) => {
...
public deleteItemConfirmed() :void {
  this.showDeleteConfirmationModal = false;
  this.geographyFeaturesService.deleteGeographicalFeature(this.eventToDelete).subscribe(
    () => {
      this.features = this.features.filter((item) => item.id !== this.eventToDelete);
...
private  onUpdateGeoFeature(updateFeature:Feature):void{
   ...
   this.geographyFeaturesService.updateGeographicalFeature(updateFeature).subscribe(()=>
     {
