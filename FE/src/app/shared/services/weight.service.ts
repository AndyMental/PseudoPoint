...
export class WeightService {
  ...
  convertWeight(weightRequest: WeightConversionRequest): Observable<WeightConversionResponse[]> {
    return this.http.post<WeightConversionResponse[]>(`${this.apiUrl}/weight_conversion`, weightRequest);
  }
  

  getCelestial(): Observable<any> {
    return this.http.get<any>('http://127.0.0.1:8000/weight_conversion/');
  }

  getWeightForCelestialObject(celestial: string, weight: number): Observable<WeightConversionResponse> {
    const url = `${this.apiUrl}/${celestial}?weight=${weight}`;
    return this.http.get<WeightConversionResponse>(url);
  }

  getCelestialFactors(): Observable<WeightConversionResponse[]> {
    const url=`${this.apiUrl}/`
    return this.http.get<WeightConversionResponse[]>(this.apiUrl);
  }

  addCelestialFactor(celestial_object: string, weight: number): Observable<any> {
  ...
  deleteCelestialFactor(celestial: string): Observable<any> {
    const url = `${this.apiUrl}/delete_celestial_factor/${celestial}`;
    return this.http.delete<any>(url);
  }

  editCelestial(celestial: string, data: WeightConversionResponse): Observable<WeightConversionResponse> {
